...
const saveDraft: NextApiHandler = async (req, res) => {
  ...
  try {
    ...
    const { title, content, slug, meta, focusKeyword, thumbnailUrl, isDirectPost, faqs: faqsRaw, postAuthorId } = fields as any;
    ...
    const postId = (fields as any).postId;
      ...
      if (postId) {
        ...
        // category đã bỏ, không cập nhật nữa
      ...
      const newDraft = new Post({
        ...
        meta: meta || "",
        tags,
        // category đã bỏ, không sử dụng nữa
