...
const AuthorPage: NextPage<Props> = ({ author, posts = [] }) => {
  ...
  const metaTitle = `${author.name} — Tác giả tại Đồng phục UniVi`;
  const metaDesc =
    author.bio ||
    `Đọc các bài viết của ${author.name}${author.role ? `, ${author.role}` : ""} tại Đồng phục UniVi.`;

  return (
    <DefaultLayout title={metaTitle} desc={metaDesc} thumbnail={author.avatar}>
      <Head>
...
export const getServerSideProps: GetServerSideProps<Props, { slug: string }> =
  async ({ params }) => {
    try {
      ...
      return { props: { author: authorData, posts } };
    } catch (error) {
