...
export default function InternalLinks({ 
  ...
  const mainCategories = [
    { name: 'Đồng phục Gym', slug: '/san-pham/dong-phuc-gym' },
    { name: 'Đồng phục Yoga - Pilates', slug: '/san-pham/dong-phuc-yoga-pilates' },
    { name: 'Đồng phục Chạy bộ', slug: '/san-pham/dong-phuc-chay-bo' },
    { name: 'Đồng phục Pickleball', slug: '/san-pham/dong-phuc-pickleball' },
    { name: 'Đồng phục MMA', slug: '/san-pham/dong-phuc-mma' },
    { name: 'Đồng phục Golf - Tennis', slug: '/san-pham/dong-phuc-golf-tennis' },
    { name: 'Đồng phục áo Polo', slug: '/san-pham/dong-phuc-ao-polo' },
    { name: 'Đồng phục công sở', slug: '/san-pham/dong-phuc-cong-so' },
    { name: 'Đồng phục Team building', slug: '/san-pham/dong-phuc-team-building' },
    { name: 'Đồng phục áo gió', slug: '/san-pham/dong-phuc-ao-gio' },
  ];

  // Lọc các danh mục liên quan (loại bỏ danh mục hiện tại)
  const currentCategorySlug = currentCategory ? currentCategory.replace('/san-pham/', '') : '';
  const availableCategories = mainCategories.filter(cat => {
    const catSlug = cat.slug.replace('/san-pham/', '');
    ...
    if (relatedCategories.some(related => {
      const relatedSlug = related.slug.replace('/san-pham/', '');
        ...
        <p className="text-sm text-gray-600 leading-relaxed">
          {productName ? (
            ...
            <>
              ...
              từ <Link href="/san-pham/dong-phuc-gym" className="text-[#105d97] hover:underline font-medium">đồng phục gym</Link>, 
              <Link href="/san-pham/dong-phuc-chay-bo" className="text-[#105d97] hover:underline font-medium">đồng phục chạy bộ</Link> đến 
              <Link href="/san-pham/dong-phuc-cong-so" className="text-[#105d97] hover:underline font-medium">đồng phục công sở</Link>. 
